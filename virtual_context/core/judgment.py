...
def candidate_tag_pairs(
    ...
    for i, a in enumerate(unique):
        for b in unique[i + 1:]:
            ...
            ratio = matcher.ratio() if (shared or matcher.quick_ratio() >= min_similarity) else 0.0
            if shared or ratio >= min_similarity:
                overlap = len(shared) / max(1, len(tokens[a] | tokens[b]))
                scored[(a, b)] = max(ratio, 0.5 + 0.5 * overlap)
    if embed_fn is not None and len(unique) >= 2:
        try:
            import numpy as np
            vecs = np.array(embed_fn(unique), dtype=float)
            ...
            for i in range(len(unique)):
                for j in range(i + 1, len(unique)):
                    ...
                    if sim >= min_similarity:
                        ...
                        scored[key] = max(scored.get(key, 0.0), sim)
        except Exception:
