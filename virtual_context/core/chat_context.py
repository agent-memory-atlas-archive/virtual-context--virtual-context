...
from .responses_context import VC_BLOCK_RE, strip_vc_text


def _without_block(content):
    if isinstance(content, str):
        return strip_vc_text(content) if VC_BLOCK_RE.search(content) else content
...
def place_context_block(messages: list, prepend_text: str) -> None:
    ...
    messages[:] = [m for m in messages if not (isinstance(m, dict) and (m.get("content") in ("", []) or (isinstance(m.get("content"), str) and not m["content"].strip())) and m.get("role") in ("system", "user") and not m.get("tool_calls"))]
