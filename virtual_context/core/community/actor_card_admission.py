...
class ActorCardAdmissionService:
    ...
    def admit_entries(
        ...
        *,
        ...
        def _parse_admission(text: str) -> tuple[bool, dict[str, dict]]:
            return parse_admission_response(
                text, parse_json=self._compactor._parse_response, eligible=eligible,
            )
...
ADMISSION_COVERAGE_REASONS: tuple[str, ...] = (
    "substantive", "greeting_only", "one_off_trivia", "bot_meta_or_test",
    "no_durable_context", "insufficient_evidence",
)
ADMISSION_REASONS: tuple[str, ...] = (
    "durable", "temporary", "test_probe", "stopped_or_replaced", "completed", "expired",
    "contradicted", "insufficient_evidence", "not_durable", "not_person_card", "wrong_subject",
    "wrong_kind", "irrelevant_citation", "redundant", "explicit_privacy_request", "agent_refused",
...
def build_admission_request(
    *,
    ...
    return {
        ...
        "max_tokens": max(800, min(4000, 300 + 250 * len(candidates))),
        "payload": payload,
    }


def parse_admission_response(
    text: str, *, parse_json: Callable[[str], dict], eligible: Iterable[str],
) -> tuple[bool, dict[str, dict]]:
    try:
        parsed = parse_json(text)
    except Exception as exc:
        raise _ActorCardAdmissionError("actor-card admission response is not valid JSON", text) from exc
    if (
        ...
        raise _ActorCardAdmissionError("actor-card admission response has invalid coverage shape", text)
    independently_substantive = parsed["substantive"]
    if parsed["coverage_reason"] not in ADMISSION_COVERAGE_REASONS or independently_substantive != (
        parsed["coverage_reason"] == "substantive"
    ):
        raise _ActorCardAdmissionError("actor-card admission response has invalid coverage decision", text)
    ...
    for decision in parsed["decisions"]:
        if (
            ...
            or decision.get("reason") not in ADMISSION_REASONS
            or (bool(decision.get("admit")) != (decision.get("reason") == "durable"))
            or decision["candidate_id"] in decisions
        ):
            raise _ActorCardAdmissionError("actor-card admission response contains an invalid decision", text)
        decisions[decision["candidate_id"]] = decision
    if set(decisions) != set(eligible):
        raise _ActorCardAdmissionError("actor-card admission response does not cover every candidate", text)
